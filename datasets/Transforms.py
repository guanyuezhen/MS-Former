...
class Scale(object):
    ...
    def __call__(self, img, patch_label, label):
        '''
        :param img: RGB image
        ...
        patch_label = cv2.resize(patch_label, (self.w, self.h), interpolation=cv2.INTER_NEAREST)
...
class RandomCropResize(object):
    ...
    def __call__(self, img, patch_label, label):
        if random.random() < 0.5:
            ...
            patch_label_crop = patch_label[y1:h - y1, x1:w - x1]
            label_crop = label[y1:h - y1, x1:w - x1]

            img_crop = cv2.resize(img_crop, (w, h))
            patch_label_crop = cv2.resize(patch_label_crop, (w, h), interpolation=cv2.INTER_NEAREST)
            label_crop = cv2.resize(label_crop, (w, h), interpolation=cv2.INTER_NEAREST)

            return img_crop, patch_label_crop, label_crop
        else:
            return [img, patch_label, label]
...
class Normalize(object):
    ...
    def __call__(self, image, patch_label, label):
        ...
        image = image / 255
        patch_label = np.ceil(patch_label / 255)
...
class ToTensor(object):
    ...
    def __call__(self, image, patch_label, label):
        if self.scale != 1:
            ...
            patch_label = cv2.resize(patch_label, (int(w / self.scale), int(h / self.scale)), interpolation=cv2.INTER_NEAREST)
        ...
        patch_label_tensor = torch.LongTensor(np.array(patch_label, dtype=int)).unsqueeze(dim=0)
        label_tensor = torch.LongTensor(np.array(label, dtype=int)).unsqueeze(dim=0)

        return [image_tensor, patch_label_tensor, label_tensor]
